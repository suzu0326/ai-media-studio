...
# Your API key (hardcoded temporarily to eliminate env var issues)
REPLICATE_API_KEY = "r8_4X9ZRVMvMt97AlwVkdSudQqD6OAq01Vr0mhEsm"

# Full HTML (complete, no truncation)
HTML = """<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .card{background:#fff;border-radius:24px;padding:30px;margin-bottom:20px;box-shadow:0 20px 40px rgba(0,0,0,0.2)}
        .card h2{color:#667eea;margin-bottom:20px}
        textarea{width:100%;padding:12px;border:2px solid #e0e0e0;border-radius:12px;font-size:16px;font-family:inherit;resize:vertical}
        button{width:100%;background:#667eea;color:#fff;border:none;padding:14px;border-radius:12px;font-size:16px;font-weight:600;cursor:pointer;margin-top:15px}
        button:hover{background:#5a67d8}
        .result{margin-top:20px;text-align:center;min-height:200px}
        .result img{max-width:100%;border-radius:12px}
        .loading{text-align:center;padding:30px}
        .loader{display:inline-block;width:40px;height:40px;border:4px solid #f3f3f3;border-top:4px solid #667eea;border-radius:50%;animation:spin 1s linear infinite}
        @keyframes spin{0%{transform:rotate(0deg)}100%{transform:rotate(360deg)}}
        .error{background:#ffe0e0;color:#c33;padding:12px;border-radius:12px}
        .footer{text-align:center;margin-top:20px;color:#fff;font-size:12px}
        .sample-images{display:flex;gap:10px;margin-top:15px;flex-wrap:wrap}
        .sample-btn{background:#f0f0f0;padding:8px 12px;border-radius:20px;font-size:12px;cursor:pointer}
    </style>
</head>
<body>
<div class="container">
    <div class="header"><h1>🎨 AI Media Studio</h1><p>Generate stunning images with AI — Powered by Flux</p></div>
    <div class="card">
...
<script>
    ...
    async function generate(){
        ...
        try{
            ...
            if(data.success) resultDiv.innerHTML=`<img src="${data.image_url}" alt="Generated Image">`;
            else resultDiv.innerHTML=`<div class="error">❌ ${data.error}</div>`;
...
def generate():
    ...
    try:
        # Force the client to use the hardcoded key
        ...
        # prediction is a list. The first element may be a string or a FileOutput.
        # We'll convert it safely.
        if hasattr(prediction[0], 'url'):
